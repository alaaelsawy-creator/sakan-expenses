...
def load_persons():
    try:
        d=requests.get(SCRIPT+"?type=persons",timeout=10).json()
        return sorted([x["name"] for x in d],key=lambda n:next((x["order"] for x in d if x["name"]==n),99))
    except: return []

def load_settings():
    try: return requests.get(SCRIPT+"?type=settings",timeout=10).json()
    except: return {}
...
@st.cache_data(ttl=30)
def load_cl():
    try: return requests.get(SCRIPT+"?type=cleaning",timeout=15).json()
    except: return []

@st.cache_data(ttl=30)
def load_gas():
    try: return requests.get(SCRIPT+"?type=gas",timeout=15).json()
    except: return []
...
SHABAB   = load_persons()
all_data = load_data()
vac_all  = load_vac_sheet()   # الحقيقة من Sheet مباشرة
...
with tab1:
    if not SHABAB: st.info("أضف أشخاصاً أولاً.")
        ...
        c=[st.columns(5)[i] for i in range(5)]
        for ci,val,lbl in zip(c,[tot_exp,total_rent,rpp,tot_exp+total_rent,f"{ac}/{len(SHABAB)}"],
                              ["💰 إجمالي المصاريف","🏠 إجمالي الإيجار","👤 إيجار الفرد","📊 الإجمالي الكلي","👥 المتواجدون"]):
            ci.markdown(f'<div class="stat-card"><div class="value">{val:.3f if isinstance(val,float) else val}</div><div class="label">{lbl}</div></div>',unsafe_allow_html=True)
...
<b>العودة من الإجازة:</b> سجّلها في تبويب الإجازات أو استخدم خانة العودة أدناه — سيدخل الجدول من الجمعة التي تليها.
        ...
        if not SHABAB:
            ...
            if st.button("💾 حفظ دور هذه الجمعة",type="primary",use_container_width=True,key="save_cl"):
                if not saved_sec or not saved_fir:
                    st.warning("⚠️ لا يوجد بيانات للحفظ.")
                elif saved_sec==saved_fir:
                    st.warning("⚠️ يجب أن يكون الشخصان مختلفَين.")
                else:
                    if not saved_nxt_sec: saved_nxt_sec=saved_sec
                    if not saved_nxt_fir: saved_nxt_fir=saved_fir
                    cleaner_str=saved_sec+"، "+saved_fir
                    next_str   =saved_nxt_sec+"، "+saved_nxt_fir
